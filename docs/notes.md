...
**重装很便宜。** 指向没变时（也就是绝大多数「改完代码装一下」）它不重启 SwiftBar，只替换文件再推一次 `swiftbar://refreshallplugins`——插件目录根部的文件被替换会触发 `DirectoryObserver` 自动重扫。只有真要改 `PluginDirectory` 才走 kill + 写偏好 + 重开那条路。

**这个脚本没有自动化测试。** CI 只做 `bash -n` 和 `shellcheck`，都是静态的；65 项回归测试一条都不覆盖它。两条分支是手动验的：把插件目录指到一个不存在的路径再跑 `./install.sh` 恢复，走的是「改指向」分支（诊断报告从 `Loaded Plugins: 0` 回到 `1`）；紧接着再跑一次，SwiftBar 的 PID 没变，走的是「不重启」分支。改这个脚本时别指望 CI 兜底，照着这两步手验一遍。

## 菜单栏上没有它，不一定是它没跑

2026-08-22 装完副本、诊断报告 `Loaded Plugins: 1`、直接跑脚本也正常出数，但菜单栏上就是没有那条天气。插件没问题，是 **macOS 把它吃掉了**。

带刘海的 MacBook 上，状态栏图标从右往左排，排到刘海就停——放不下的直接不画，不报错、不提示。当时的数字（`NSScreen.auxiliaryTopRightArea` 实测）：刘海右缘在 848pt，右侧第一个图标在 891pt，只剩 43pt；而「☂ 17-19时雨」在 13pt 菜单栏字体下要 90pt 左右。

**为什么偏偏是它。** SwiftBar 拿插件的绝对路径当 `autosaveName`，`install.sh` 把路径从仓库换成 `Application Support` 之后，在 macOS 眼里这是一个全新的状态栏项——没有记住的位置，排到最左，正好撞上刘海。不过「谁被吃」并不严格等于「谁最新」：实测临时退出另一个菜单栏 App 腾出位置后，天气冒出来了，但一个更早就在的项反而不见了。规则是系统的，只能确定「放不下就一定有项看不见」，别指望精确预测是哪个。

**每块屏各自排。** 外接屏没有刘海、1920pt 宽，所有项都放得下，天气一直在那儿。只有单用笔记本屏时才会遇到这个问题。

排查顺序里要加一条：诊断报告说加载了、脚本跑得出东西、菜单栏却空着 → 先看是不是刘海。最快的确认是看外接屏，或者临时退出一个别的菜单栏 App 看它会不会冒出来。

解法不在插件这边，两条：⌘ 拖动把它挪到别的图标右边（位置按 `autosaveName` 记住，**以后 `install.sh` 换了路径就得再拖一次**）；或者减少菜单栏上的项——刘海右边就那么宽，不减项就永远有看不见的。菜单栏管理器（Ice / Bartender）是第三条路，但它的模型是把东西收进隐藏区点开才看，跟「抬眼一次就知道要不要带伞」这个设计初衷相反，本项目不推荐。
...
加 `install.sh` 的时候这条规则立刻咬了一口：它在仓库根目录，非隐藏、不是 `.json`、非空，是个**合法插件候选**——插件目录一旦被指到仓库根就会被 `chmod +x` 然后执行，而这脚本里有 `killall SwiftBar`，等于自杀重启循环。是测试 G2 当场抓到的。

修的时候还踩了第二下：第一版写的是 `*.sh`，G3 马上红了——那条测试的阳性对照 `real.5m.sh` 是个 shell 插件。**SwiftBar 插件本来就大量是 `.sh`，拿扩展名 glob 去排自己的脚本，会连真插件一起挡掉。** 正确写法是精确文件名 `install.sh`，它走「先精确比对文件名」那一臂，同样任意深度生效。

一般化的教训：往仓库根目录添任何**可执行的辅助脚本**时都过一遍这个问题，并且排除它时用文件名而不是扩展名。
